...
import { useAnswerAnimator } from "../hooks/useAnswerAnimator";
...
export function ChatShell() {
  ...
  const answerAnimator = useAnswerAnimator(dispatch);
    ...
    onEvent(event) {
      ...
      switch (event.type) {
        ...
        case "chat.progress":
          ...
          break;
        case "answer.delta":
          answerAnimator.enqueue(requestId, event.delta);
          break;
        case "answer.sources":
          answerAnimator.setSources(requestId, event.sources);
          break;
        case "answer.products":
          answerAnimator.setProducts(requestId, event.products);
          break;
        case "answer.done":
          answerAnimator.complete(requestId, event);
          break;
        case "error":
          answerAnimator.reset();
              ...
              {state.progress && (
                ...
                >
                  ...
                  <span className="progress-bubble">
                    {state.progressTrail.map((step, index) => {
                      const active = index === state.progressTrail.length - 1;
                      return (
                        <span
